  ...
  return (
    <CardContent>
      <div className="space-y-4">
        ...
        {data.selectedCrops.filter(crop => !requiredCrops.map(c => c.id).includes(crop)).length > 0 && (
          <div className="pt-2">
            <Label>Your Custom Crops:</Label>
            <div className="flex flex-wrap gap-2 mt-2">
              {data.selectedCrops
                .filter(crop => !requiredCrops.map(c => c.id).includes(crop))
                .map(crop => (
                  <div 
                    key={crop} 
